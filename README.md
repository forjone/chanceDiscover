...
- **Phase 1 · 自动采集** — App Store RSS（免 key）与 Google Play 抓取，按应用名搜索后一键采集；**CSV/TSV 批量导入**（自动识别中英文列名）；**增量水位线**采集（只取上次之后的新评论，去重幂等）
- **Phase 2 · 监控与趋势** — 为每个痛点关键词记录真实热度轨迹并可视化；**定时监控**（Cron 接口 + 开关）自动刷新矿源并重新挖掘；每次采集 / 挖掘都留存执行记录，支撑纵向积累

## 产品化能力

- **通知与周报** — 自动生成周报（可下载/查看），Webhook 推送（Slack / 飞书 / Discord / 钉钉 / 任意 JSON 端点）；监控运行时对**新高分机会自动预警**（阈值可调，每个机会只提醒一次）
- **评分可解释** — 机会详情每个维度可展开「为什么是这个分」，给出基于真实数据的依据（声量/情绪/付费信号/竞品分布/真实热度来源）
- **自动标签建议** — 按痛点主题与信号启发式推荐标签（搜索/稳定性/离线/变现/高潜力…），一键添加
- **落地工具箱** — 机会一键生成 **PRD / 落地页文案 / MVP 任务清单 / 联网反向尽调**（Claude 生成，反向尽调用 web_search 联网核查竞品；缺省回退模板）
- **机会看板(Kanban)** — 按状态拖拽流转，标签管理，**协作动态**记录谁改了什么
- **向量聚类 + 痛点演化** — TF-IDF 单链聚类 + 近重复去重；跨运行追踪每个痛点评论量的升降
- **机会工作流** — 状态流转（新发现 / 观察中 / 在做了 / 已归档）、标签、私人笔记，重新挖掘后按标题**自动延续**
